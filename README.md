...
指令： /mnt/zihanw/proj_utils_pro/run_interactive.sh
可用的投影项目:
...
3. 关于每个seg的帧数，目前的模型接受的是29帧，所以每个seg帧数要输入29帧，不要选默认值。seg的数量（这取决于有多少帧，我们可以把它切段切成几个29帧，就输入几。例如在上一步第5点，比如选择产了90帧数据，那那么就可以产出来3个29帧的seg，seg数量输出为“3”）和视频帧率这些默认值是符合cosmos post training demo的配置，默认1280 720p，生成在/mnt/zihanw/proj_utils_pro/transfer_video_maker/output（！记得修改这个绝对路径），主要分类依据不是场景号，是根据transfer控制头分类的。
